...
Create an object
  ...
  $ rebind input.js | tee output.ml
  type 
  external make : name:string -> id:float -> unit ->  = ""[@@mel.obj ]

  $ melc -ppx melppx output.ml
  File "output.ml", line 2, characters 0-8:
  2 | external make : name:string -> id:float -> unit ->  = ""[@@mel.obj ]
      ^^^^^^^^
  Error: Syntax error
  [2]
...
Non-mutating instance method
  ...
  type str
  type replace
  external replace : str -> substr -> newSubstr -> replace = "replace"[@@mel.send
                                                                      ]
