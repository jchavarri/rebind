...
let from_lb_p source start _end =
  Lexing.
    {
      source;
      start =
...
let btwn_exclusive loc1 loc2 =
  { source = loc1.source; start = loc1._end; _end = loc2.start }
...
let char_before loc =
  let start =
    ...
    let column, offset =
      if column > 0 then (column - 1, offset - 1) else (column, offset)
    in
    { line; column; offset }
  in
...
let compare =
  ...
  fun loc1 loc2 ->
    ...
    if k = 0 then
      ...
      if k = 0 then pos_cmp loc1._end loc2._end else k
    else k
...
let with_suffix filename suffix = filename_map (fun fn -> fn ^ suffix) filename
