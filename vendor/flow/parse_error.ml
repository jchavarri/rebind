...
module PP = struct
  let error = function
    ...
    | AccessorDataProperty ->
        "Object literal may not have data and accessor property with the same \
         name"
    | AccessorGetSet ->
        "Object literal may not have multiple get/set accessors with the same \
         name"
    | StrictLHSAssignment ->
    ...
    | StrictLHSPostfix ->
        "Postfix increment/decrement may not have eval or arguments operand in \
         strict mode"
    | StrictLHSPrefix ->
        "Prefix increment/decrement may not have eval or arguments operand in \
         strict mode"
    ...
    | DeclareAsync ->
        "async is an implementation detail and isn't necessary for your \
         declare function statement. It is sufficient for your declare \
         function to just have a Promise return type."
    | DeclareExportLet ->
        "`declare export let` is not supported. Use `declare export var` \
         instead."
    | DeclareExportConst ->
        "`declare export const` is not supported. Use `declare export var` \
         instead."
    | DeclareExportType ->
        "`declare export type` is not supported. Use `export type` instead."
    | DeclareExportInterface ->
        "`declare export interface` is not supported. Use `export interface` \
         instead."
    | UnexpectedExportStarAs ->
        "`export * as` is an early-stage proposal and is not enabled by \
         default. To enable support in the parser, use the \
         `esproposal_export_star_as` option"
    | DuplicateExport export ->
        Printf.sprintf "Duplicate export for `%s`" export
    | ExportNamelessClass ->
        "When exporting a class as a named export, you must specify a class \
         name. Did you mean `export default class ...`?"
    | ExportNamelessFunction ->
        "When exporting a function as a named export, you must specify a \
         function name. Did you mean `export default function ...`?"
    | UnsupportedDecorator -> "Found a decorator in an unsupported position."
    | MissingTypeParamDefault ->
        "Type parameter declaration needs a default, since a preceding type \
         parameter declaration has a default."
    | WindowsFloatOfString ->
        "The Windows version of OCaml has a bug in how it parses hexidecimal \
         numbers. It is fixed in OCaml 4.03.0. Until we can switch to 4.03.0, \
         please avoid either hexidecimal notation or Windows."
    | DuplicateDeclareModuleExports ->
        "Duplicate `declare module.exports` statement!"
    | AmbiguousDeclareModuleKind ->
        "Found both `declare module.exports` and `declare export` in the same \
         module. Modules can only have 1 since they are either an ES module \
         xor they are a CommonJS module."
