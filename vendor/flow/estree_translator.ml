...
module Translate
  ...
  val program :
    Loc.t * Ast.Statement.t list * (Loc.t * Ast.Comment.t') list -> t
...
with type t = Impl.t = struct
  ...
  let option f = function Some v -> f v | None -> null

  let position p =
    obj [| ("line", int p.Loc.line); ("column", int p.Loc.column) |]
  ...
  let range location =
    Loc.(array [| int location.start.offset; int location._end.offset |])
  ...
  let errors l =
    let error (location, e) =
      obj
        [|
          ("loc", loc location); ("message", string (Parse_error.PP.error e));
        |]
    in
  ...
  let rec program (loc, statements, comments) =
    node "Program" loc
      [|
        ("body", statement_list statements); ("comments", comment_list comments);
      |]
  ...
  and statement =
    Statement.(
      ...
      | loc, While _while ->
          While.(
            node "WhileStatement" loc
              [|
                ("test", expression _while.test); ("body", statement _while.body);
              |])
      | loc, DoWhile dowhile ->
          DoWhile.(
      ...
      | loc, ForIn forin ->
          ForIn.(
            let left =
              match forin.left with
              | LeftDeclaration left -> variable_declaration left
              | LeftExpression left -> expression left
            in
      ...
      | loc, ForOf forof ->
          ForOf.(
            let type_ =
              if forof.async then "ForAwaitStatement" else "ForOfStatement"
            in
            let left =
              match forof.left with
              | LeftDeclaration left -> variable_declaration left
              | LeftExpression left -> expression left
            in
            node type_ loc
      ...
      | loc, DeclareModule m ->
          DeclareModule.(
            ...
            node "DeclareModule" loc
              ...
              |])
      | loc, DeclareExportDeclaration export -> (
      ...
      | loc, ImportDeclaration import ->
          ImportDeclaration.(
            let specifiers =
              import.specifiers
              |> List.map (function
                   | ImportDefaultSpecifier id -> import_default_specifier id
    ...
    Expression.(
      ...
      | loc, Array arr ->
          node "ArrayExpression" loc
            [|
              ( "elements",
                array_of_list (option expression_or_spread) arr.Array.elements
              );
            |]
      | loc, Object _object ->
          node "ObjectExpression" loc
            [|
              ( "properties",
                array_of_list object_property _object.Object.properties );
            |]
      | loc, Function _function -> function_expression (loc, _function)
      | loc, ArrowFunction arrow ->
          Function.(
            let body =
              match arrow.body with
              | BodyBlock b -> block b
      ...
      | loc, Sequence sequence ->
          node "SequenceExpression" loc
            [|
              ( "expressions",
                array_of_list expression sequence.Sequence.expressions );
            |]
      | loc, Unary unary -> (
          Unary.(
      ...
      | loc, Assignment assignment ->
          Assignment.(
            let operator =
              match assignment.operator with
              | Assign -> "="
              | PlusAssign -> "+="
              | MinusAssign -> "-="
              | MultAssign -> "*="
      ...
      | loc, Update update ->
          Update.(
            let operator =
              match update.operator with Increment -> "++" | Decrement -> "--"
            in
      ...
      | loc, Logical logical ->
          Logical.(
            let operator =
              match logical.operator with Or -> "||" | And -> "&&"
            in
  ...
  and function_declaration (loc, fn) =
    Function.(
      let body =
        match fn.body with
        | BodyBlock b -> block b
        | BodyExpression b -> expression b
      in
  ...
  and function_expression (loc, _function) =
    Function.(
      let body =
        match _function.body with
        | BodyBlock b -> block b
        | BodyExpression expr -> expression expr
      in
  ...
  and catch (loc, c) =
    Statement.Try.CatchClause.(
      node "CatchClause" loc
        [| ("param", pattern c.param); ("body", block c.body) |])

  and block (loc, b) =
    node "BlockStatement" loc
      [| ("body", statement_list b.Statement.Block.body) |]

  and declare_variable (loc, d) =
    Statement.DeclareVariable.(
      let id_loc =
        Loc.btwn (fst d.id)
          (match d.typeAnnotation with
  ...
  and class_implements (loc, implements) =
    Class.Implements.(
  ...
  and class_body (loc, body) =
    Class.Body.(
      node "ClassBody" loc [| ("body", array_of_list class_element body.body) |])

  and class_element =
    Class.Body.(
      function Method m -> class_method m | Property p -> class_property p)
  ...
  and interface_declaration (loc, i) =
    Statement.Interface.(
    ...
    Pattern.(
      ...
      | loc, Object obj ->
          node "ObjectPattern" loc
            [|
              ( "properties",
                array_of_list object_pattern_property obj.Object.properties );
              ( "typeAnnotation",
                option type_annotation obj.Object.typeAnnotation );
            |]
      | loc, Array arr ->
          node "ArrayPattern" loc
            [|
              ( "elements",
                array_of_list (option array_pattern_element) arr.Array.elements
              );
      ...
      | loc, Assignment { Assignment.left; right } ->
          node "AssignmentPattern" loc
            [| ("left", pattern left); ("right", expression right) |]
  ...
  and object_pattern_property =
    Pattern.Object.(
      ...
      | RestProperty (loc, prop) ->
          RestProperty.(
            node "RestProperty" loc [| ("argument", pattern prop.argument) |]))
  ...
  and comprehension_block (loc, b) =
    Expression.Comprehension.Block.(
    ...
    Literal.(
      ...
      let props =
        ...
        | RegExp { RegExp.pattern; flags } ->
            let regex =
              obj [| ("pattern", string pattern); ("flags", string flags) |]
            in
  ...
  and template_literal (loc, value) =
  ...
  and template_element (loc, element) =
    Expression.TemplateLiteral.Element.(
      let value =
        obj
          [|
      ...
      node "TemplateElement" loc
        [| ("value", value); ("tail", bool element.tail) |])

  and tagged_template (loc, tagged) =
  ...
  and variable_declaration (loc, var) =
    Statement.VariableDeclaration.(
      let kind =
        match var.kind with Var -> "var" | Let -> "let" | Const -> "const"
      in
  ...
  and variance (_, sigil) =
    Variance.(
      match sigil with Plus -> string "plus" | Minus -> string "minus")
  ...
  and nullable_type loc t =
    node "NullableTypeAnnotation" loc [| ("typeAnnotation", _type t) |]

  and function_type (loc, fn) =
    Type.Function.(
  ...
  and function_type_param (loc, param) =
    Type.Function.Param.(
  ...
  and object_type_property (loc, prop) =
  ...
  and object_type_indexer (loc, indexer) =
    Type.Object.Indexer.(
  ...
  and object_type_call_property (loc, callProperty) =
  ...
  and array_type loc t =
    node "ArrayTypeAnnotation" loc [| ("elementType", _type t) |]

  and generic_type_qualified_identifier (loc, q) =
    Type.Generic.Identifier.(
      let qualification =
        match q.qualification with
        | Unqualified id -> identifier id
        | Qualified q -> generic_type_qualified_identifier q
      in
      node "QualifiedTypeIdentifier" loc
        [| ("qualification", qualification); ("id", identifier q.id) |])

  and generic_type (loc, g) =
    Type.Generic.(
      let id =
        match g.id with
        | Identifier.Unqualified id -> identifier id
  ...
  and union_type (loc, ts) =
    node "UnionTypeAnnotation" loc [| ("types", array_of_list _type ts) |]
  ...
  and typeof_type (loc, t) =
    node "TypeofTypeAnnotation" loc [| ("argument", _type t) |]

  and tuple_type (loc, tl) =
    node "TupleTypeAnnotation" loc [| ("types", array_of_list _type tl) |]

  and string_literal_type (loc, s) =
    Type.StringLiteral.(
      node "StringLiteralTypeAnnotation" loc
        [| ("value", string s.value); ("raw", string s.raw) |])

  and number_literal_type (loc, s) =
    Type.NumberLiteral.(
      node "NumberLiteralTypeAnnotation" loc
        [| ("value", number s.value); ("raw", string s.raw) |])

  and boolean_literal_type (loc, s) =
    Type.BooleanLiteral.(
      node "BooleanLiteralTypeAnnotation" loc
        [| ("value", bool s.value); ("raw", string s.raw) |])

  and exists_type loc = node "ExistsTypeAnnotation" loc [||]

  and type_annotation (loc, ty) =
    node "TypeAnnotation" loc [| ("typeAnnotation", _type ty) |]
  ...
  and type_parameter_instantiation (loc, params) =
    Type.ParameterInstantiation.(
      node "TypeParameterInstantiation" loc
        [| ("params", array_of_list _type params.params) |])

  and jsx_element (loc, (element : JSX.element)) =
    JSX.(
      node "JSXElement" loc
        [|
  ...
  and jsx_closing (loc, closing) =
    JSX.Closing.(
      node "JSXClosingElement" loc [| ("name", jsx_name closing.name) |])
  ...
  and jsx_attribute (loc, attribute) =
    JSX.Attribute.(
      let name =
      ...
      node "JSXAttribute" loc
        [|
          ("name", name); ("value", option jsx_attribute_value attribute.value);
        |])
  ...
  and jsx_spread_attribute (loc, attribute) =
    JSX.SpreadAttribute.(
      node "JSXSpreadAttribute" loc
        [| ("argument", expression attribute.argument) |])

  and jsx_expression_container (loc, expr) =
    JSX.ExpressionContainer.(
      let expression =
        match expr.expression with
        | Expression expr -> expression expr
        | EmptyExpression empty_loc -> node "JSXEmptyExpression" empty_loc [||]
      in
      node "JSXExpressionContainer" loc [| ("expression", expression) |])

  and jsx_text (loc, text) =
    JSX.Text.(
      let { value; raw } = text in
      node "JSXText" loc [| ("value", string value); ("raw", string raw) |])

  and jsx_member_expression (loc, member_expression) =
    JSX.MemberExpression.(
      let _object =
        match member_expression._object with
        | Identifier id -> jsx_identifier id
        | MemberExpression member -> jsx_member_expression member
      in
  ...
  and import_named_specifier local_id remote_id =
    let span_loc =
      ...
      | None -> fst remote_id
    in
    let local_id = match local_id with Some id -> id | None -> remote_id in
    node "ImportSpecifier" span_loc
      [| ("imported", identifier remote_id); ("local", identifier local_id) |]
  ...
  and comment (loc, c) =
    Comment.(
      let _type, value =
        match c with Line s -> ("Line", s) | Block s -> ("Block", s)
      in
